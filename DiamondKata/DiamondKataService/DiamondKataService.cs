...
namespace DiamondKata.Services
{
    public class DiamondKataService
    {
        ...
        public string Execute(char character)
        {
            ...
            for(int n=1; n<=alphaPosition;n++){ //loop to get each line up to and including the middle row
                sb.AppendLine(PrintLine(((char)(64+n)), lineLength));
            }

            //now complte the lower row
            for(int n=alphaPosition-1; n>=1;n--){ //loop to get each line up to and including the middle row
                sb.AppendLine(PrintLine(((char)(64+n)), lineLength));
            }
